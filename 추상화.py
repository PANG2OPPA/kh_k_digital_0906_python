# 추상화
# 부모 클래스에서 선언한 메소드에 대해서 반드시 상속받은 클래스에서 기능을 구현해야 하는 특성
# 추상 메소드가 포함된 부모 클래스는 객체로 만들 수 없고 단지 상속을 주기 위해서 존재
from abc import *

class NetworkAdapter(metaclass=ABCMeta): # 추상 클래스 선언
    @abstractmethod
    def connect(self): # 구현 할 내용 없는 경우 pass 키워드 사용
        pass
...
class LAN(NetworkAdapter):
    ...
    def connect(self): # 부모에게서 상속 받은 추상 메소드를 구현 함
